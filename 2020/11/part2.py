...
def count_occupied_neighbours(row, column):
    ...
    for row_delta in [-1, 0, 1]:
        for column_delta in [-1, 0, 1]:
            ...
            # keep going in a straight line until a seat is reached
            while 0 <= neighbour_row < height and 0 <= neighbour_column < width and grid[neighbour_row][neighbour_column] == ".":
                neighbour_row = neighbour_row + row_delta
                neighbour_column = neighbour_column + column_delta
...
while not stable: 
    ...
    for row in range(height):
        for column in range(width):
            ...
            if grid[row][column] == "#" and occupied_neighbours >= 5:
